(() => {
  ...
  const notesKey = 'dronlab_sts_bloque7_notes';
  let current = 0;
  const esc = value => String(value ?? '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  function slideTemplate(item, index) {
    const comp = item.id === 'b7-10' ? '<div class="callout"><b>Trazabilidad complementaria:</b> MME8/geocaging y masas STS-ES nacionales quedan separados del Syllabus AESA pagina 11.</div>' : '';
    if (index === 0) return `<section class="slide active" data-id="${esc(item.id)}"><div class="hero-grid"><div><p class="eyebrow">BLOQUE 7 STS</p><h1>${esc(data.meta.titulo)}</h1><p class="subtitle">${esc(data.meta.subtitulo)}</p><div class="hero-tags"><span>AESA ${esc(data.meta.paginas_relacionadas)}</span><span>${esc(data.meta.estado)}</span><span>MTOM / CG / Payload</span></div></div><div class="meteo-visual" aria-hidden="true"><div class="cloud"><b>UAS</b><span>MTOM | CG | Payload | Envolvente</span></div></div></div><div class="cue">${esc(item.docente)}</div><aside class="notes"><b>Notas de clase</b><textarea data-note="${esc(item.id)}"></textarea></aside></section>`;
    return `<section class="slide" data-id="${esc(item.id)}"><p class="eyebrow">Bloque 7 STS</p><h2>${esc(item.titulo)}</h2><div class="compare"><article><div class="badge">OBJETIVO</div><h3>${esc(item.titulo)}</h3><p>${esc(item.contenido)}</p></article><article><div class="badge alt">TRAZA</div><h3>Syllabus STS</h3><p>Temario oficial AESA | Paginas relacionadas: ${esc(data.meta.paginas_relacionadas)} | Estado ${esc(data.meta.estado)}</p></article></div>${comp}<div class="callout"><b>Profesor Plus:</b> ${esc(item.docente)}</div><div class="cue">${esc(item.docente)}</div><aside class="notes"><b>Notas de clase</b><textarea data-note="${esc(item.id)}"></textarea></aside></section>`;
  }
