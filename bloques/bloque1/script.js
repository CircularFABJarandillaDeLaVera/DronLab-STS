(() => {
  ...
  const notesKey = 'dronlab_sts_bloque1_notes';
  ...
  function renderSlide(slide, i) {
    ...
    const body = i === 0
      ? `<div class="hero-grid"><div class="hero-copy"><p class="eyebrow">DRONLAB STS</p><h1>${escapeHtml(title)}</h1><p class="subtitle">${escapeHtml(objective)}</p><div class="institutional-pill"><img src="../../assets/branding/logo-circular-fab.png" alt=""><span>Circular FAB Jarandilla de la Vera</span></div><div class="hero-tags"><span>Riesgo</span><span>SORA</span><span>STS</span><span>Espacio aereo</span></div></div><div class="drone-visual" aria-label="Diagrama STS"><div class="system-card"><span class="icon">STS</span><b>Bloque Base</b><small>Reglamentacion</small></div></div></div>`
      : `<p class="eyebrow">BLOQUE 1 STS</p><h2>${escapeHtml(title)}</h2><div class="compare compact"><article><div class="badge">OBJETIVO</div><h3>${escapeHtml(title)}</h3><p>${escapeHtml(content)}</p></article><article><div class="badge alt">TRAZA</div><h3>Syllabus STS</h3><p>Temario oficial AESA · Paginas relacionadas: ${escapeHtml(data.trazabilidad.paginas_relacionadas)} · Estado ${escapeHtml(data.trazabilidad.estado)}</p></article></div><div class="callout"><b>Profesor Plus:</b> ${escapeHtml(teacher)}</div>`;
    return `
      <section class="slide ${i === 0 ? 'active slide-hero' : ''}" data-title="${escapeHtml(title)}" data-section="Bloque 1 STS">
        ${body}
        <div class="cue">${escapeHtml(teacher)}</div>
        <aside class="notes">
          <b>Que decir:</b> ${escapeHtml(content)}<br>
          <b>Preguntas:</b> Que decision operacional cambia con este concepto?<br>
          <b>Respuestas:</b> Vincular la decision con la ingenieria validada y el Syllabus STS.<br>
          <b>Error:</b> Mezclar declaracion, autorizacion, STS y PDRA.<br>
          <b>Reconducir:</b> Volver a la trazabilidad AESA paginas 3-4.
        </aside>
      </section>
    `;
  }
  ...
  function render() {
    ...
    document.title = `${index + 1}. ${all[index].dataset.title} - DronLab STS`;
  }
