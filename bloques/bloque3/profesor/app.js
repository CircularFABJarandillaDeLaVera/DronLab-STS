  ...
  function show(i) {
    ...
    c.innerHTML = `
      <article class="card hero">
        <div class="tags"><span class="tag">Pantalla ${esc(s.id)}</span><span class="tag">Bloque 3 STS</span><span class="tag">${esc(d.trazabilidad.estado)}</span></div>
        <h1>${esc(s.titulo)}</h1>
        <p>${esc(s.contenido)}</p>
      </article>
      <article class="card script"><h2>Que decir</h2><p>${esc(s.docente || s.contenido)}</p></article>
      <div class="grid">
        <article class="card box"><h3>Preguntas</h3>${list(['Que procedimiento corresponde aqui?', 'Que debe verificar el piloto antes o durante la operacion?'])}</article>
        <article class="card box"><h3>Respuestas esperadas</h3>${list(['Respuesta apoyada en la ingenieria STS validada.', 'Referencia a paginas 5-6 del Syllabus STS cuando proceda.'])}</article>
        <article class="card box"><h3>Error habitual</h3><p>Confundir STS-01 y STS-02, geocaging y FTS, o contingencia y emergencia.</p></article>
        <article class="card box"><h3>Como reconducir</h3><p>Volver a la secuencia: pre-vuelo, contingencia, emergencia, ERP.</p></article>
        <article class="card box"><h3>Demostracion</h3><p>Plantear una intrusion o salida de volumen y pedir al alumnado que clasifique la respuesta.</p></article>
