(() => {
  ...
  function normalize(json) {
    ...
    return {
      titulo: json.titulo || 'Test Bloque 3 STS',
      bloque: json.bloque || 'Procedimientos operacionales - Syllabus STS paginas 5-6 - Estado VALIDADO',
    ...
    main.innerHTML = `
      ...
      ${visible.map((question, index) => `
        <section class="panel question" data-id="${question.id}">
          ...
          <div class="feedback"><p><b>Explicacion:</b> ${question.explicacion}</p><p><b>Error habitual:</b> ${question.error_habitual}</p><p><b>Referencia:</b> ${question.referencia_concepto || 'Syllabus STS paginas 5-6 - Estado VALIDADO'}</p></div>
        </section>
      `).join('')}
    `;
