(() => {
  ...
  function normalize(json) {
    ...
    return {
      titulo: json.titulo || 'Flashcards Bloque 4 STS',
  ...
  function render() {
    ...
    main.innerHTML = `<section class="panel hero"><h1>${data.titulo}</h1><p>${data.bloque}</p><div class="stats"><span class="pill">${data.total} tarjetas</span><span class="pill">${stats.facil || 0} faciles</span><span class="pill">${stats.media || 0} medias</span><span class="pill">${stats.dificil || 0} dificiles</span></div></section><section class="flashgrid">${visible.length ? visible.map(cardHtml).join('') : '<p class="panel empty">No hay tarjetas para este filtro.</p>'}</section>`;
  ...
  fetch('flashcards.json').then(response => response.json()).then(json => {
    data = normalize(json);
